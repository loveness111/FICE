...
def prepare_compass_dataset(filename, path_data):
    ...
    # Fill missing values in length_of_stay using the median value
    df['length_of_stay'].fillna(df['length_of_stay'].median(), inplace=True)

    # Fill missing values in days_b_screening_arrest using the median value
    ...
    print("Continuous Values: %s \nDiscrete values: %s" % (continuous, discrete))
    ...
    X = df_le.loc[:,
        df_le.columns != class_name].values
