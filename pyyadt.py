...
def predict_rule(dt, x, class_name, features_type, discrete, continuous):
    ...
    y, tree_path = predict_single_record(dt, x, class_name, edge_labels, node_labels, node_isleaf, features_type,
                                         discrete, continuous)
    if y is None:
...
def yadt_value2type(x, attribute, features_type):
    try:
        ...
        print(f"Warning: Cannot convert {x} to numeric value for attribute {attribute}")
        x = None
...
def get_counterfactuals(dt, tree_path, rule, diff_outcome, class_name, continuous, features_type):
    ...
    print("Node diff outcome path:", node_diff_outcome_path)
    ...
    for ctp in node_diff_outcome_path:
        ...
        print("Counterfactual condition:", ccond)
        delta, qlen = get_falsifeid_conditions1(cond, ccond,
                                                continuous)
        print("Delta:", delta)
        print("QLen:", qlen)
        if qlen < clen:
        ...
        print("Counterfactuals so far:", counterfactuals)
...
def get_falsifeid_conditions1(cond, ccond, continuous):
    qlen = 0
    fcond = {}
    for att, c_val in ccond.items():
        if att in continuous:
            if att not in cond:
                # If the attribute does not exist in the original conditions
                min_c, max_c = c_val
                if min_c > -np.inf and max_c < np.inf:
                    fcond[att] = f"{min_c} < {att} <= {max_c}"
                    qlen += 2
                elif min_c > -np.inf:
                    fcond[att] = f">{min_c}"
                    qlen += 1
                ...
                # If the attribute exists in both conditions, compare their ranges
                r_val = cond[att]
                min_r, max_r = r_val
                min_c, max_c = c_val

                new_min = max(min_r, min_c) if min_c > min_r else min_r
                new_max = min(max_r, max_c) if max_c < max_r else max_r

                if new_min < new_max:
                    fcond[att] = f"{new_min} < {att} <= {new_max}"
                    qlen += 2
                elif new_min == new_max:
                    if new_min > min_r or new_max < max_r:
                        fcond[att] = f"{att} == {new_min}"
                        qlen += 1

        else:
            if att not in cond or c_val != cond[att]:
                fcond[att] = c_val
                qlen += 1

    print(f"Final falsified conditions: {fcond}, Query Length: {qlen}")
    return fcond, qlen
...
def apply_counterfactual(x, delta, continuous, discrete, features_type):
    ...
    for att, val in delta.items():
        ...
        try:
            ...
            print(f"Error applying counterfactual for attribute {att}: {e}")
