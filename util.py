...
def recognize_features_type(df, class_name):
    ...
    integer_features.extend(['days_b_screening_arrest', 'length_of_stay'])
...
def set_discrete_continuous(columns, type_features, class_name, discrete=None,
                            continuous=None):
...
def label_decode(df, columns, label_encoder):
    ...
    for col in columns:
        if col not in label_encoder:
            print(f"Warning: No label encoder found for column '{col}'. Skipping decoding.")
            continue
        le = label_encoder[col]
        try:
            df_de[col] = le.inverse_transform(df_de[col])
        except ValueError as e:
            print(f"Warning: Failed to decode column '{col}': {e}")
...
def get_closest_diffoutcome1(df, x, discrete, continuous, class_name, blackbox, label_encoder, distance_function,
                             k=100, diff_out_ratio=0.1):
    distances = []
    idx = 0
    Z, _ = label_encode(df, discrete, label_encoder)
    ...
    for d, idx, z1 in distances:
        if blackbox.predict(z1.reshape(1, -1))[0] == 0:
            indexes0.append(idx)
        else:
            indexes1.append(idx)

    print("The number of neighborhood data points with a prediction result of 0:", len(indexes0))
    print("The number of neighborhood data points with a prediction result of 1:", len(indexes1))

    k_index = int(k * diff_out_ratio)
    # final_indexes = distances[:k_index]
    final_indexes = [item[1] for item in distances[:k_index]]

    print("The actual number of neighborhood data points selected:", len(final_indexes))

    for idx in indexes0[:k - k_index]:
        final_indexes.append((0, idx, Z[idx]))
    for idx in indexes1[:k - k_index]:
        final_indexes.append((0, idx, Z[idx]))

    final_indexes = [item[1] for item in final_indexes]
...
def build_df2explain(bb, X, dataset):
    ...
    for i, col in enumerate(columns):
        ...
        data_col = data_col.astype(int) if features_type[
                                               col] == 'integer' else data_col
        data.append(data_col)

    # print("Data before transposing:", data)
